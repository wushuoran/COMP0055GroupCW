...
class poisoner(object):
    ...
    def __init__(self, train_x, train_y, test_x, test_y, valid_x, valid_y, step, beta, sigma, eps):
        ...
        self.sigma = sigma
        self.eps = eps
        self.classifier = linear_model.ElasticNetCV(max_iter=10000)
        self.classifier.fit(self.train_x, self.train_y)
        self.lam = self.classifier.alpha_
        self.classifier = linear_model.ElasticNet(alpha = self.lam,max_iter=10000,warm_start=True)
    ...
    def poison_data(self, x_pois, y_pois, stop_1, stop_2, stop3, decrease_rate):
        ...
        while True:
            ...
            self.classifier.fit(np.vstack((self.train_x, x_pois)), self.train_y + y_pois)
            ...
            for i in range(poison_ct):  # deal with each poisoned data element
                ...
                result = (self.classifier.predict(self.train_x) - self.train_y)
                errors = self.classifier.predict(self.train_x) - self.train_y
                l1 = (np.dot(errors, self.train_x) * (-1)) / self.row_amt
                l2 = np.matrix(weight).reshape(1, self.col_amt)
                # regularization parameter. used to prevent over-fitting
                reg = self.lam * (l1 + l2) * 0.5 + np.zeros((1, self.col_amt))
                ...
                attack_x = np.dot(result, grad_x) / self.row_amt + np.dot(reg, weight_expt_last_row) * self.lam
                attack_y = np.dot(result, grad_y) / self.row_amt + np.dot(reg, weight_expt_last_col.T) * self.lam
            ...
            ''' train the new model on the new poisoned data and get the loss (Line 9 in Algorithm 1)'''
    ...
    def line_search(self, current_x, current_y, current_x_pois_ele, current_y_pois_ele, attack_vals, attack_bias):
        ...
        loss_before = self.loss_function(classifier_copy,lam)
        ...
        while count < 99 and dif > eps:
            ...
            loss_after = self.loss_function(classifier_copy,lam)
    ...
    def loss_function(self, classifier, lam):
        ...
        l2 = np.linalg.norm(classifier.coef_, 2) / 2
        errs = classifier.predict(self.train_x) - self.train_y
        init_loss = (np.linalg.norm(errs) ** 2) / self.row_amt
        return (lam * (l1 + l2)) / 2 + init_loss
    ...
    def compute_weight_bias(self, weight, matrix, row_amt, x_pois_ele):
        ...
        sigma = self.lam * np.eye(self.col_amt) * 0.5
